...
operator+(любой_тип);   // для временного StringN, дописывает без лишнего копирования
operator+=(любой_тип);  // изменяет текущую строку
operator=(любой_тип);
...
String32 str("abc");
Serial.println(str + "def");       // abcdef, str не изменится
Serial.println("value=" + str);    // value=abc, str не изменится
...
- Оператор `+=` изменяет исходную строку
- Оператор `+` для временного `StringN` дописывает данные прямо в него и возвращает тот же временный объект. Это позволяет эффективно собирать строки без лишних промежуточных буферов: `foo(String64() + 123 + "hello")`
- Для обычного именованного `StringN` оператор `+` не изменяет исходную строку: работают `StringN + "text"`, `"text" + StringN` и `StringN + StringN`
- После первого немутирующего сложения результат является временным `StringN`, поэтому дальнейшая цепочка снова собирается без лишних копирований: `foo("value=" + str + 123 + "abc")`
